...
describe("GatewaySession Security Analysis", function () {
  ...
  beforeEach(async function () {
    [owner, attacker, victim, provider, otherProvider] =
    ...
    await token
      .connect(victim)
      .approve(await trustEngine.getAddress(), parseUSDC(10000));
    await trustEngine
      .connect(victim)
      .deposit(await token.getAddress(), parseUSDC(1000));
  ...
  describe("1. Gateway Registry Attacks", function () {
    it("should prevent gateway slug hijacking after registration", async function () {
      ...
      await gatewaySession
        .connect(provider)
        .registerGateway("popular-api", parseUSDC(0.01));

      // Attacker tries to register same slug
      await expect(
        gatewaySession
          .connect(attacker)
          .registerGateway("popular-api", parseUSDC(0.001)),
    ...
    it("should prevent unauthorized gateway deactivation", async function () {
      await gatewaySession
        .connect(provider)
        .registerGateway("victim-api", parseUSDC(0.01));
    ...
    it("should prevent unauthorized price manipulation", async function () {
      await gatewaySession
        .connect(provider)
        .registerGateway("stable-api", parseUSDC(0.01));

      // Attacker tries to change price to drain funds faster
      await expect(
        gatewaySession
          .connect(attacker)
          .updateGatewayPrice("stable-api", parseUSDC(1000)),
      ).to.be.revertedWith("Not gateway owner");
    });
  });
  ...
  describe("2. Session Manipulation Attacks", function () {
    beforeEach(async function () {
      await gatewaySession
        .connect(provider)
        .registerGateway("test-api", parseUSDC(0.01));
    });
    ...
    it("should prevent provider from recording usage on other provider's session", async function () {
      ...
      await gatewaySession
        ...
        .registerGateway("other-api", parseUSDC(0.01));
      ...
      await expect(
        gatewaySession
          .connect(otherProvider)
          .recordUsage(sessionId, parseUSDC(50)),
      ).to.be.revertedWith("Only provider or syncer can record usage");
    });
  ...
  describe("3. Time-Based Attacks", function () {
    beforeEach(async function () {
      await gatewaySession
        .connect(provider)
        .registerGateway("time-api", parseUSDC(0.01));
    });
    ...
    it("should prevent usage recording after session expiry", async function () {
      ...
      const tx = await gatewaySession
        .connect(victim)
        .openSession("time-api", await token.getAddress(), parseUSDC(100), 60);
      const sessionId = (await tx.wait())?.logs.find(
        (l) => l.fragment?.name === "SessionOpened",
      )?.args?.sessionId;
    ...
    it("should allow settlement of expired session by anyone", async function () {
      ...
      await gatewaySession
        .connect(provider)
        .recordUsage(sessionId, parseUSDC(30));
  ...
  describe("4. Economic Attacks", function () {
    beforeEach(async function () {
      await gatewaySession
        .connect(provider)
        .registerGateway("econ-api", parseUSDC(0.01));
    });
    ...
    it("should prevent draining via multiple small sessions", async function () {
      ...
      const balance = await trustEngine.balances(
        attacker.address,
        await token.getAddress(),
      );
    ...
    it("should correctly handle dust amounts in fee calculation", async function () {
      ...
      const victimBalanceBefore = await trustEngine.balances(
        victim.address,
        await token.getAddress(),
      );
      await gatewaySession.connect(victim).settleSession(sessionId);
      const victimBalanceAfter = await trustEngine.balances(
        victim.address,
        await token.getAddress(),
      );
  ...
  describe("5. Session ID Security", function () {
    beforeEach(async function () {
      await gatewaySession
        .connect(provider)
        .registerGateway("id-api", parseUSDC(0.01));
    });
    ...
    it("should generate unique session IDs even for same parameters", async function () {
      ...
      for (let i = 0; i < 5; i++) {
        const tx = await gatewaySession
          .connect(victim)
          .openSession("id-api", await token.getAddress(), parseUSDC(10), 3600);
        const sessionId = (await tx.wait())?.logs.find(
          (l) => l.fragment?.name === "SessionOpened",
        )?.args?.sessionId;
    ...
    it("should not allow operations on non-existent session IDs", async function () {
      const fakeSessionId = ethers.keccak256(
        ethers.toUtf8Bytes("fake-session"),
      );

      await expect(
        gatewaySession
          .connect(provider)
          .recordUsage(fakeSessionId, parseUSDC(10)),
      ).to.be.revertedWith("Session not active");
  ...
  describe("6. Fund Isolation", function () {
    beforeEach(async function () {
      await gatewaySession
        .connect(provider)
        .registerGateway("isolation-api", parseUSDC(0.01));
    });

    it("should isolate funds between different sessions", async function () {
      // Victim opens session
      const tx1 = await gatewaySession
        .connect(victim)
        .openSession(
          "isolation-api",
          await token.getAddress(),
      ...
      const session1 = (await tx1.wait())?.logs.find(
        (l) => l.fragment?.name === "SessionOpened",
      )?.args?.sessionId;

      // Attacker opens separate session
      const tx2 = await gatewaySession
        .connect(attacker)
        .openSession(
          "isolation-api",
          await token.getAddress(),
      ...
      const session2 = (await tx2.wait())?.logs.find(
        (l) => l.fragment?.name === "SessionOpened",
      )?.args?.sessionId;
      ...
      await gatewaySession
        .connect(provider)
        .recordUsage(session2, parseUSDC(100));
