...
describe("GatewaySession", function () {
  ...
  describe("Gateway Registration", function () {
    it("should register a gateway", async function () {
      ...
      await expect(
        gatewaySession
          .connect(provider)
          .registerGateway("my-api", pricePerRequest),
      )
        .to.emit(gatewaySession, "GatewayRegistered")
        .withArgs("my-api", provider.address, pricePerRequest);

      const [gatewayProvider, gatewayPrice] = await gatewaySession.getGateway(
        "my-api",
      );
    ...
    it("should reject duplicate gateway registration", async function () {
      ...
      await expect(
        gatewaySession.connect(other).registerGateway("my-api", 2000),
    ...
    it("should allow gateway owner to update pricing", async function () {
      ...
      await gatewaySession.connect(provider).registerGateway("my-api", 1000);
      await expect(
        gatewaySession.connect(provider).updateGatewayPrice("my-api", 2000),
  ...
  describe("Session Lifecycle", function () {
    async function setupWithGateway() {
      ...
      await fixture.gatewaySession
        .connect(fixture.provider)
        .registerGateway("test-api", pricePerRequest);
    ...
    it("should open a session", async function () {
      ...
      expect(
        await trustEngine.balances(agent.address, await token.getAddress()),
      ).to.equal(DEPOSIT_AMOUNT - sessionDeposit);
    });

    it("should record usage", async function () {
      const { gatewaySession, token, agent, provider, pricePerRequest } =
        await setupWithGateway();
      ...
      await expect(
        gatewaySession
          .connect(provider)
          .recordUsage(sessionId, pricePerRequest),
      )
        .to.emit(gatewaySession, "UsageRecorded")
        .withArgs(sessionId, pricePerRequest, pricePerRequest);

      // Verify usage accumulated
      expect(await gatewaySession.getRemainingCredits(sessionId)).to.equal(
        sessionDeposit - pricePerRequest,
      );
    });

    it("should settle session and distribute funds", async function () {
      const { gatewaySession, trustEngine, token, agent, provider } =
        await setupWithGateway();
      ...
      await gatewaySession
        .connect(provider)
        .recordUsage(sessionId, usageAmount);

      // Get balances before settlement
      const agentBalanceBefore = await trustEngine.balances(
        agent.address,
        await token.getAddress(),
      );
      const providerBalanceBefore = await trustEngine.balances(
        provider.address,
        await token.getAddress(),
      );
      ...
      expect(
        await trustEngine.balances(provider.address, await token.getAddress()),
    ...
    it("should allow agent to cancel session with no usage", async function () {
      const { gatewaySession, trustEngine, token, agent, DEPOSIT_AMOUNT } =
        await setupWithGateway();
    ...
    it("should reject usage exceeding deposit", async function () {
      ...
      await expect(
        gatewaySession.connect(provider).recordUsage(sessionId, excessiveUsage),
