...
describe("ValidationBridge", function () {
  ...
  beforeEach(async function () {
    ...
    await validationBridge
      .connect(owner)
      .authorizeValidator(validator1.address);
  });
  ...
  describe("TrustEngine Bridge", function () {
    it("should request validation for a deal", async function () {
      ...
      await expect(
        validationBridge
          .connect(agent1)
          .requestValidation(dealId, agent1Id, uri, hash),
      )
    ...
    it("should record validation result", async function () {
      ...
      await validationBridge
        .connect(agent1)
        .requestValidation(dealId, agent1Id, "uri", reqHash);
      ...
      await expect(
        validationBridge
          .connect(validator1)
          .recordValidation(reqHash, score, resUri, resHash, tag),
      )
    ...
    it("should execute conditional release", async function () {
      ...
      await validationBridge
        .connect(validator1)
        .recordValidation(reqHash, 95, "uri", ethers.ZeroHash, ethers.ZeroHash);
      ...
      await expect(
        validationBridge.executeConditionalRelease(dealId, 80),
  ...
  describe("ERC-8004 Functions", function () {
    it("should handle ERC-8004 validation request/response", async function () {
      ...
      await expect(
        validationBridge
          .connect(agent1)
          .validationRequest(validator1.address, agent1Id, uri, reqHash),
      )
      ...
      const summary = await validationBridge.getSummary(
        agent1Id,
        [],
        ethers.ZeroHash,
      );
