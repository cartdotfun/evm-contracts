...
describe("ReputationRegistry", function () {
  ...
  describe("Basic Feedback", function () {
    it("should post feedback", async function () {
      ...
      await expect(
        reputationRegistry
          .connect(client1)
          .postFeedback(agent1Id, score, skillTag, uri, hash, dealId),
      )
      ...
      const overall = await reputationRegistry.getSummary(
        agent1Id,
        ethers.ZeroHash,
      );
    ...
    it("should get feedback details", async function () {
      ...
      await reputationRegistry
        .connect(client1)
        .postFeedback(agent1Id, 90, ethers.ZeroHash, "", hash, ethers.ZeroHash);
  ...
  describe("ERC-8004 Feedback", function () {
    it("should give feedback with valid signature", async function () {
      ...
      expect(
        await reputationRegistry.getLastIndex(agent1Id, client1.address),
      ).to.equal(1);

      const fb = await reputationRegistry.readFeedback(
        agent1Id,
        client1.address,
        1,
      );
    ...
    it("should revoke feedback", async function () {
      ...
      await expect(
        reputationRegistry.connect(client1).revokeFeedback(agent1Id, 1),
      )
    ...
    it("should append response", async function () {
      ...
      expect(
        await reputationRegistry.getResponseCount(
          agent1Id,
          client1.address,
          1,
          [],
    ...
    it("should read all feedback with filtering", async function () {
      ...
      const all = await reputationRegistry.readAllFeedback(
        agent1Id,
        [],
        ethers.ZeroHash,
        ethers.ZeroHash,
      ...
      const filtered = await reputationRegistry.readAllFeedback(
        agent1Id,
        [],
        tagA,
        ethers.ZeroHash,
      ...
      expect(await reputationRegistry.getClients(agent1Id)).to.deep.equal([
        client1.address,
      ]);
    });
  });
  ...
  describe("Admin", function () {
    ...
    it("should only allow owner to set identity registry", async function () {
      await expect(
        reputationRegistry.connect(agent1).setIdentityRegistry(agent1.address),
